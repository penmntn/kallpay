...
import VuePerfectScrollbar from 'vue-perfect-scrollbar'
    ...
    export default {
        ...
        computed: {
            ...
            side_active: {
                get(){
                    for(let i=0; i < this.$store.state.siderbarTest.length ; i++){
                        if(this.$store.state.siderbarTest[i] === this.identificador) return true
                    }
                    return false
                },
                set(val){
                    console.log(val)
                    let payload = { id : this.identificador , state : val}
                    this.$store.commit('UPDATE_SIDER', payload)
                }
            },
            count_indice(){
                return this.$store.state.siderbarTest.length
